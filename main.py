...
parser.add_argument('--cli', help="path to simple switch cli", type=str, action="store", default = "simple_switch_CLI")
...
parser.add_argument('--pcap-dump', help='Dump packets on interfaces to pcap files',
					action="store_true")
# No use for the --manifest argument since the p4app.json file is always in the same directory
parser.add_argument('--manifest', '-m', help='Path to manifest file',
					type=str, action="store", default = "p4app.json")
...
def main():
	...
	if args.auto_control_plane or 'controller_module' in conf:
		controller = AppController(topo_file=conf['topo_file'], 
				net=net, cli_path=args.cli, log_dir = args.log_dir,
				sw_path = args.behavioral_exe)
	net.start()
	sleep(1)
